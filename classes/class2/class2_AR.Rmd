...
### Extra Class Notes -- Live Coding Session
```{r}

# Function
newFunction <- function(x){ #anything can be there in place of x, just prioritise readability
  print("heyyyy im calculating stuff wait a moment")
  new_x = x + 2
  return(new_x)
}
newFunction(2)
...
# defining parameters
mu = 0.0
sigma = 1.0 # can also use arrows... same thing.
tau = 1.0

# f1 function, define f1 -- following the slide for now..
f1 <- function(x) {
...
# define f2
f2 <- function(x) {
  sqrt(tau / 2 * pi) * exp(-tau * ((x-mu)/sigma))^2
}

# generating sequences
xVal <- seq(-5, # start
            5, # end
            by = 0.1) # incremental steps

# let's plug something in!! lets calc some y values using the functions f1 and f2 on our generated x's
y_f1 <- f1(xVal)
y_f2 <- f2(xVal)

# create df f/ plotting
...
plot1 <- df %>% 
  ...
  labs(title = "Probability Desnity Functions with mu = 0, sigma = 1, tau = 1",
       caption = "methods 2 is fun :)",
       y = "Probability Density") +
  theme_minimal()

plot1

# let's create a column to check if they're the same :,)
## if they are, they are probably diff.
df <- df %>% 
  mutate(checking_stuff = 
           ifelse(f1 == f2, TRUE, FALSE)) 
# THEY SHOULD BE THE SAME HERE, BUT THEY ARE NOT SO WE NEED TO FIX IT WITH PERNILLE'S CODE

# now do the same with diff. parameters
mu = ## can also directly input these into the functions themselves so you can define them there
  ### i am nauseous
  # fx. f1 <- function(x = xVal, sigma = 3, mu = 4, tau = 3) -- role of each, tau = precision, sigma = std
  ### precision, being certain, usually conveyed with a a curve that is narrow - the taller and more narrow, the more certain it is, the wider it is, we're not sure about mean...  lower std narrow curve, higher std wider curve
  #### input the photo from phone to capture
  
# portfolio 1 : simulations + probability, functions, for-loops, will be online tomorrow -> due March 10th, don't panic ab/ desc., will be covered on Monday lecture!

# - - - - -

# shift + ctrl + a on windows (clean up of code)
...
* for-loop = we can execute a set of statements, once for each item in a vector/array/list
** does NOT require an indexing var. to set beforehand, like w/ while loops
** works as an iterator method

* while-loop = can execute a set of statements as long as a condition is TRUE
** REMEMBER: increment i, or else the loop will continue forever

### Ex. B: When would you use for-loops and when would you use while-loops? Explain in your own words in 1-2 sentences

* for loop is used when no. of iterations is know, while loop until statement in program is proved wrong.. 
...
while (some_number < 20) {
  print(paste(some_number, "the number is below 20, so I still run!"))
  if ((some_number + 7) < 20) {
    some_number <-  some_number + 7 # yes, oh so simple..
  } else {
    break
  }
} # why is it this complicated..???? how am i ever to learn???
```
...
temp_celsius <- convert_to_celsius(68) # yay it works!!!
print(temp_celsius)
...
start w/ exercise 0 and go to 5.5 for the sake of portfolio -- highly recommended by Pernille :)
...
p_success <- 0.4 #the probability of making a shot

sim_shots <- function(){ 
  consecutive_misses <- 0
  shots_taken <- 0 # function simulates the player's shots until they miss two shots in a row. It keeps track of the number of consecutive misses and the total number of shots taken
  while(consecutive_misses < 2){
    shots_taken <- shots_taken + 1 # In each iteration of the while loop, the function generates a random number using runif(1) and compares it with p_success to determine if the shot is successful or not
  if (runif(1) > p_success){
    consecutive_misses <- consecutive_misses + 1
  } else {
    consecutive_misses <- 0
  } 
  } # If a shot is missed, consecutive_misses is incremented by 1. Otherwise, it's reset to 0
  return(shots_taken)
} # function returns the total number of shots taken before two consecutive misses occur.

n_sims <- 1000
results <- replicate(n_sims, sim_shots()) # simulation is repeated n_sims times using replicate, and the results are stored in the results variable

avg_shots <- mean(results)
cat("Average No. of Shots Taken Before 2 Consecutive Misses:", avg_shots, "\n") # average number of shots taken before two consecutive misses is calculated and printed
...
basketball <- function(p, x, y, z){ # starting the function for you
